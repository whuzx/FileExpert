...
public abstract class FtpCmd implements Runnable {
	...
	public static File inputPathToChrootedFile(File existingPrefix, String param, String encoding) {
		String newParam;
		try {
			newParam = new String(param.getBytes(encoding), encoding);
		} catch (UnsupportedEncodingException e1) {
			return inputPathToChrootedFile(existingPrefix, param);
		}
		try {
			if(newParam.charAt(0) == '/') {
				// The STOR contained an absolute path
				File chroot = Globals.getChrootDir();
				return new File(chroot, newParam);
			}
		} catch (Exception e) {} 
		
		// The STOR contained a relative path
		return new File(existingPrefix, newParam); 
	}
